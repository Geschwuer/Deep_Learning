...
class FullyConnected(BaseLayer):
    ...
    def initialize(self, weights_initializer, bias_initializer):
        # fan_in = input_size of w and fan_out = output_size of w
        fan_in = self.weights.shape[0] - 1  # -1 because you have to remove the bias
        fan_out = self.weights[1]
        ...
        bias = bias_initializer.initialize(
            (1, fan_out), fan_in, fan_out
        )

        self.weights = np.vstack(weights, bias)     # add bias to weights vector
